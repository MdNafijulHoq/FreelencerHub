...
const AllJobs = () => {
    ...
    useEffect(( () => {
        const getData = async () => {
            const { data } = await axios.get(`${import.meta.env.VITE_API_URL}/all-jobs?page=${currentPage}&size=${itemPerPage}&filter=${filter}&sort=${sort}&search=${search}`)
            setJobs(data)
          }
      getData()
    ...
    const perPage = Math.ceil(count / itemPerPage)

    const pages = [...Array(perPage).keys().map(element => element + 1)]
