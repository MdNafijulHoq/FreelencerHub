...
const BidRequest = () => {
  ...
  const handleStatus = async(id, prevStatus, status) => {
    ...
    if(prevStatus === status) return toast.error('You can not submit twice!')
                        ...
                        bids.map(bid => <tr key={bid._id}>
                              ...
                              <div className='flex items-center gap-x-2'>
                                <p
                                  className={`px-3 py-1 rounded-full text-blue-500 bg-blue-100/60
                                   text-xs ${bid.category === 'Web Development' && 'text-blue-500 bg-blue-100/60'} ${bid.category === 'Graphics Design' && 'text-emerald-500 bg-emerald-100/60'} 
                            ...
                            <td className='px-4 py-4 text-sm font-medium text-gray-700 whitespace-nowrap'>
                              <div className={`inline-flex items-center px-3 py-1 rounded-full gap-x-2 ${bid.status === 'Pending' && 'bg-yellow-100/60 text-yellow-500'} ${bid.status === 'In Progress' && 'bg-blue-100/60 text-blue-500'} ${bid.status === 'Complete' && 'bg-emerald-100/60 text-emerald-500'} ${bid.status === 'Rejected' && 'bg-red-100/60 text-red-500'}`}>
                                <span className={`h-1.5 w-1.5 rounded-full ${bid.status === 'Pending' && 'bg-yellow-500'} ${bid.status === 'In Progress' && 'bg-blue-500'} ${bid.status === 'Complete' && 'bg-emerald-500'} ${bid.status === 'Rejected' && 'bg-red-500'}`}></span>
